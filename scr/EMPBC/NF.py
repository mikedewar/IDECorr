...
class NF():
	...
	def gen_ssmodel(self):
		...
		t0=time.time()
		simulation_space_x_y=pb.arange(-30,30.5,0.5)
	...
	def simulate(self,T):
		...
		#v_membrane1=pb.reshape(v0,(sim_field_space_len,sim_field_space_len))
		firing_rate=self.act_fun.fmax/(1.+pb.exp(self.act_fun.varsigma*(self.act_fun.v0-v_membrane)))
		#firing_rate1=self.act_fun.fmax/(1.+pb.exp(self.act_fun.varsigma*(self.act_fun.v0-v_membrane)))
		for t in T[1:]:
			...
			#temp1=pb.hstack((firing_rate,firing_rate,firing_rate))
			#temp2=pb.vstack((temp1,temp1,temp1))
			#g1=signal.convolve2d(self.K,temp2,mode='full')
			#g1=g1[61:102,61:102] 
			#g=signal.fftconvolve(self.K,firing_rate,mode='same') 
			g=signal.convolve2d(self.K,firing_rate,mode='same',boundary='wrap') 
			g*=(self.spacestep**2)
			#g1*=(self.spacestep**2)
			v_membrane=self.Ts*g +self.xi*v_membrane+w
			#v_membrane1=self.Ts*g1 +self.xi*v_membrane1+w
			firing_rate=self.act_fun.fmax/(1.+pb.exp(self.act_fun.varsigma*(self.act_fun.v0-v_membrane)))
			#firing_rate1=self.act_fun.fmax/(1.+pb.exp(self.act_fun.varsigma*(self.act_fun.v0-v_membrane1)))
			#Observation
			temp1=pb.hstack((v_membrane[:,1:],v_membrane,v_membrane[:,:-1]))
			temp2=pb.vstack((temp1[1:,:],temp1,temp1[1:,:]))
			Y.append((self.spacestep**2)*(pb.dot(self.C,pb.reshape(temp2,(121**2,1))))+v)
			#Y.append((self.spacestep**2)*(pb.dot(self.C,pb.reshape(v_membrane,(sim_field_space_len**2,1))))+v)
			V.append(v_membrane)
			#V1.append(v_membrane1)
