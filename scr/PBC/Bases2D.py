...
class basis():
	...
	def __add__(self,In):
		...
		'''adds two Gaussian basis functions

		Arguments
		---------
			In: basis instant or lambda function
		Returns:
		-------
			out: lambda function'''

		if (In.__class__ is basis) or (In.__class__ is 'function'):'must be a Gaussian basis function'

		output=lambda X,Y:self.__call__(X,Y)+In.__call__(X,Y)
		return output

	def __radd__(self,In):
	...
	def __or__(self,In):
		...
		'''calculates product of two basis functions
		Arguments:
		----------
			In: basis instance
		Returns:
...
'''
		...
		width=(self.width*In.width)/(self.width+In.width)
		mu=(self.width*In.mu+In.width*self.mu)/(self.width+In.width)
		rij=(self.mu-In.mu).T*(1./(self.width+In.width))*(self.mu-In.mu)
		constant=float(self.constant*In.constant*pb.exp(-rij))
		return basis(mu,width,self.dimension,constant)
