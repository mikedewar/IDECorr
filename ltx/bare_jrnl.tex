...
\subsection{Example I: isotropic mixing kernel}
 Consider the following homogeneous mixing kernel with $n_\theta=2$, $\boldsymbol\mu_0=\boldsymbol\mu_1=\mathbf 0$, $\sigma_0^2=3.24$, $\sigma_1^2=5.76$, and $\theta_0=0.5$, $\theta_1=-0.3$ in equations \ref{eq:sumofGaussians} and \ref{eq:Kernelbasis}. The kernel and its estimation are plotted in  \figurename{\ref{fig:MexHatKernel2d}}. Diagonal cross-sections of the true and estimated kernels are also shown in \figurename{\ref{fig:MexHatKernel1d}}. The estimated disturbance covariance function is shown in \figurename{\ref{fig:EstimatedSupport1d}}. In this figure the estimated covariance function shown by dashed line where $\sigma_{\varepsilon}=0$ and the value at the origin is then corrected using according to the inequality \ref{eq:BoundOnObsVariance} which gives $\sigma_{\varepsilon}^2=0.104$.
