...
\begin{center}
	...
	& $\Delta$ &Spatial discretization step&0.5&mm \\ 
 	...
 	& $T$ &Number of time steps used in simulations&20000&n.a. \\ 
 	&Model&&& \\
 	& $\varsigma$ &Slope of the sigmoidal function&0.56&mV$^{-1}$spike.s$^{-1}$ \\ 
 	& $z_0$ &Threshold of the sigmoidal function&1.8&mV \\ 
  ...
  & $\Delta_{y}$ &Distance between adjacent sensors&1.5 &mm \\  
  & $\sigma_{m}$ &Observation kernel width&0.9 &mm \\  
  & $\boldsymbol\Sigma_{\varepsilon}$ &Observation noise covariance matrix&$0.1\times\mathbf{I}_{n_y}$ &mV$^2$ \\  
	&Spatial mixing kernel&&& \\
 	&Isotropic&&& \\
& $\boldsymbol\theta$ &Vector of connectivity kernel parameters&$[80~-80~5]^{\top}$& mVspike$^{-1}$ \\
& $\boldsymbol\mu_1,\boldsymbol\mu_2, \boldsymbol\mu_3$ &Vector of kernel centers&$[0~0]^{\top}$& mm \\  
& $\sigma_{\psi_1}$, $\sigma_{\psi_2}$, $\sigma_{\psi_3}$ &Spatial kernel width parameters&$1.8, 2.4, 6$& mm \\  
 	&Anisotropic&&& \\
& $\boldsymbol\theta$ &Vector of connectivity kernel parameters&$[80~-80~5~15]^{\top}$& mVspike$^{-1}$ \\
& $\boldsymbol\mu_1,\boldsymbol\mu_2, \boldsymbol\mu_3, \boldsymbol\mu_4 $ &Vector of kernel centers&$[0~0]^{\top}, [-3~0]^{\top} $& mm \\    
& $\sigma_{\psi_1}$, $\sigma_{\psi_2}$, $\sigma_{\psi_3}$, $\sigma_{\psi_4}$  &Spatial kernel width parameters&$1.8, 2.4, 6, 2$& mm \\
...
\begin{figure*}[ht]
	...
	\caption{Estimation of the disturbance covariance function. True, estimated and cross-sections at observation locations are shown in (a), (b) and (c) respectively.}
	\label{fig:label}
\end{figure*}
...
\end{align}                                                                                                                       
This is in fact a spacial case for the general solutions given by \eqref{eq:Fourier_TF_of_Kernel} and \eqref{eq:EM-MMGFourier2} with $\varsigma=4$ and $z_0=0.5$. This can be further simplified by setting the decay parameter, $\xi=0$ to model diffusion processes and sampling time, $T_s=1$. In this case the solution for the kernel is given by
