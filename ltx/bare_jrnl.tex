...
\author{Parham Aram,
        Michael Dewar and Visakan Kadirkamanathan~\IEEEmembership{Member,~IEEE}  % <-this % stops a space
...
Wikle et al. \cite{Wikle2002} describe the IDE using a state-space formulation by decomposing the kernel and the field using a set of spectral basis functions. An alternative approach for the decomposition of the IDE was introduced in \cite{Dewar2009} where the resulting state and parameter space dimensions are independent of the number of observation locations. This is crucial not only for model parsimony but also has implications for studying systems observed at a high spatial resolution. In this method an Expectation-Maximisation (EM)  algorithm  was used to estimate both the field and the kernel from the observed data. A similar approach is adopted by \cite{Scerri2009} which developed a model selection procedure by considering various spatial scales at which to represent the system. %choose the appropriate model amongst models of varying Gaussian scale parameters incorporating spatio-temporal frequency domain properties of the data.
%  when the system spatial bandwidth and the kernel support are unknown.

Behaviour of the spatial field is often of interest at a variety of scales \cite{Wiens1989,Chelton1994,Schmidt2005,Schnell2007}. At each scale, an appropriate model should be adopted to adequately reflect the underlying phenomenon. The spatial field may have features at multiple scales either naturally or as a consequence of the measurement process \cite{Wilsky2002,BAKSHI1999} and hence modelling methods that represent the field at multiple scales are essential. Multiresolution Approximation (MRA) \cite{Meyer1993,Mallat1989a,Mallat1989b}  provides such a modelling framework in which information at different scales can be represented simultaneously.
%  and can be used for the analysis and modelling of systems in which the spatio-temporal dispersal patterns are markedly different at the global and local scales \cite{Chelton1994,WIKLE2003,Schnell2007}. At each scale, an appropriate model should be adopted to adequately reflect the underlying dynamics. 

% At the same time, a successful multiresolution representation opens up possibilities to further investigate the system in order to develop more efficient modelling schemes. For instance, in irregular observation scenario information at different scales can be integrated in a model with varying scales each suited for a specific region. %Often a proper scale is not known a priori and hence MRA can gives insight to appropriate levels is necessary to adequately capture the behaviour of the system.

% three distinct
% ways in which multiresolution features can enter into a signal or image analysis problem. 

% The spatial field may have features at multiple scales by nature,  or regardless the underlying nature of the field ,  observations might be collected at several different resolutions. In such cases, modelling methods that represent the field at multiple scales are better suited than single scale models.

% The spatial field may possess features at multiple scales by nature,  or regardless the underlying nature of the field ,  observations might be collected at several different resolutions. Therefore modelling methods that represent the field at multiple scales are better suited than single scale models. Multiresolution approximation (MRA) \cite{Meyer1993,Mallat1989a}  provides such modelling framework in which information at different scales can be obtained simultaneously. 

% One example is multiresolution representation of the gravity field in which a global trend (coarse level) is computed from the satellite data and the regional and local information (fine level) is obtained from terrestrial and airbone data sets \cite{Schmidt2005}. 

% In addition to representing a phenomenon at different scales, MRA approximation properties make them attractive for many identification problems. MRA has been implemented in both static and dynamic spatio-temporal  modelling contexts, for example in static modelling it has been used to provide a multiresolution covariance structure for non-stationary models which can adapt to heterogeneous spatial correlations \cite{D.Nychka2002}. In dynamic modelling MRA is used to identify CML models, which infer spatio-temporal patterns from measured data \cite{Billings2006}.


%   Semi-orthogonal B-spline wavelets are compactly supported and optimally localised in time and frequency. 
...
% - a property that the alternative models described above do not typically share.
% Gaussian Radial Basis Functions have been used by  to identify the appropriate scale for the IDE to represent the underlying spatio-temporal dynamic however not in a MRA framework.
...
\end{equation}
In this expansion, $\alpha_{j_0,l}$ are the approximation coefficients  at lowest scale $j_0$ and $ \beta_{j,l}$ represent the detail coefficients at different scales $j$, with $\phi_{j,l}\left(s\right)=2^{\frac{j}{2}}\phi\left(2^js-l\right) $ and $\psi_{j,l}\left(s\right)=2^{\frac{j}{2}}\psi\left(2^js-l\right)$. Integers $j$ and $l$ are called scale and translation parameters accordingly. The one-dimensional field can similarly be decomposed as
\begin{equation}
...
% however it can be tailored to capture underlying dynamics with desired accuracy for a sufficiently large value of $j$, a value between zero and three is usually sufficient in many applications \cite{Wei2006}. Note that number of translation parameters is always finite and depends on the range of measured data.
