...
\end{align}
By using similar arguments that were used in the derivation for the connectivity kernel, the auto-correlation can be simplified by recognising that
...
\end{equation}
where $\sigma_{mm\gamma}^2$ is the width of the fitted Gaussian function to the estimated support of $(m\ast m\ast\gamma)(\boldsymbol\tau)$ from \eqref{eq:EM-MMGFourier2}.
...
\subsection{One dimensional example}
 Consider the following homogeneous spatial mixing kernel 
\setlength{\arraycolsep}{0.0em}
\begin{align}
 	k\left(\mathbf{r}-\mathbf{r}'\right) &=\theta_0 \exp{\left(-\frac{(\mathbf{r}-\mathbf{r}'-\boldsymbol\mu_0)^\top(\mathbf{r}-\mathbf{r}'-\boldsymbol\mu_0)}{\sigma_0^2}\right)}\nonumber\\
&+\theta_1 \exp{\left(-\frac{(\mathbf{r}-\mathbf{r}'-\boldsymbol\mu_1)^\top(\mathbf{r}-\mathbf{r}'-\boldsymbol\mu_1)}{\sigma_1^2}\right)},
\label{eq:1DSimulationKernel}
\end{align}
\setlength{\arraycolsep}{5pt}                                                                                                             
where $\boldsymbol\mu_0=\boldsymbol\mu_1=\mathbf 0$, $\sigma_0^2=3.24$, $\sigma_1^2=5.76$, and $\theta_0=0.5$, $\theta_1=-0.3$.
