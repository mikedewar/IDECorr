...
In this paper we take a different approach, using the autocorrelation and cross-correlation of the observed field a simple method is proposed to estimate the spatial mixing kernel, filed disturbance characteristics and the observation noise variance. This eliminate the computational complexity of the methods proposed in \cite{Dewar2009,Scerri2009} where they assumed that disturbance and noise characteristics were known to the estimator. Alternatively the proposed method can be considered a prerequisite for the EM algorithm  where
the estimated kernel, disturbance and observation noise can be used to estimate the underlying field.
...
\begin{align}
	...
	&= \left(\left(m\ast k\ast z_t\right)\star y_t \right)(\boldsymbol{\tau}) + \left(\left(m\ast e_t\right)\star y_t \right)(\boldsymbol{\tau})+ \left(\varepsilon_{t+1} \star y_t\right)\left(\boldsymbol{\tau}\right).
\end{align}
...
\begin{align}
	\mathcal{F}\{(\mathbf{E}\left[R_{y_ty_t}(\boldsymbol\tau)\right] - \sigma_{\varepsilon}^2 \delta(\boldsymbol\tau))\}\ge&0 \nonumber \\
...
\end{align}
Now substituting in \eqref{eq:EM-Fourier_TF_of_Kernel} for $\mathcal{F}\{w(-\boldsymbol{\iota})\}$ results in
\begin{align}
	\mathcal{F}&\{(m\ast m \ast \gamma)(\boldsymbol\tau)\} = \mathcal{F}\{\mathbf{E}[R_{y_{t+1}y_{t+1}}(\boldsymbol{\tau})]\}\nonumber \\ 
...
\begin{align}\label{eq:EM-MMGFourier1}
	\mathcal{F}&\{m(\boldsymbol\tau)\}\mathcal{F}\{m(\boldsymbol\tau)\}\mathcal{F}\{\gamma(\boldsymbol\tau)\} = \nonumber \\
&\mathcal{F}\{\mathbf{E}[R_{y_{t+1}y_{t+1}}(\boldsymbol{\tau})]\}- \left[\frac{\mathcal{F}\{\mathbf{E}[R_{y_{t+1}y_t}(\boldsymbol{\tau})]\}\mathcal{F}\left\{\mathbf{E}\left[R_{y_ty_{t+1}}(\boldsymbol\tau)\right] \right\}}{\mathcal{F}\{(\mathbf{E}\left[R_{y_ty_t}(\boldsymbol\tau)\right]\} - \sigma_{\varepsilon}^2}\right]-\sigma_{\varepsilon}^2 ,
\end{align}
...
&\mathcal{F}^{-1}\left\lbrace \frac{1}{\tilde{m}(\boldsymbol\tau)}\left[\left[\mathcal{F}\{\mathbf{E}[R_{y_{t+1}y_{t+1}}(\boldsymbol{\tau})]\}-\sigma_{\varepsilon}^2\right]- \left[\frac{\mathcal{F}\{\mathbf{E}[R_{y_{t+1}y_t}(\boldsymbol{\tau})]\}\mathcal{F}\left\{\mathbf{E}\left[R_{y_ty_{t+1}}(\boldsymbol\tau)\right] \right\}}{\mathcal{F}\{(\mathbf{E}\left[R_{y_ty_t}(\boldsymbol\tau)\right]\} - \sigma_{\varepsilon}^2}\right]\right]\right\rbrace  ,
\end{align}
where
\begin{equation}
