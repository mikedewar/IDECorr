...
\section{Simulation}
This section demonstrates the performance of the proposed estimation scheme. Several examples are shown where different spatial mixing kernels were adopted. In each case data is generated using equations \ref{eq:ConvolutionIntegral} and \ref{eq:ObservationEquation}  and sampled at $t=1 \dots 2000$. The formulations given in \ref{eq:EM-KernelSolution} and \ref{eq:EM-MMGFourier2} are applied in two stages, first $\sigma_{\varepsilon}^2$ is set to zero as this only affects the estimation at the origin, this way we avoid numerical errors at other locations. Second $\sigma_{\varepsilon}^2$ is chosen according to the bound estabilished by \ref{eq:BoundOnObsVariance}  to correct the estimation at the origin. 
